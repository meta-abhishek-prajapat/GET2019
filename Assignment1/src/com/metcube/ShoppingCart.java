...
public class ShoppingCart {
	...
	public static void main(String[] args) throws IOException {
		...
		do{
			...
			if(choice==1){
				...
				if(result==1){
					System.out.println("Select Item Id to update= ");
					Integer itemId=input.nextInt();
					Cart itemExist=GetCartItem(itemId);
					...
					if(itemExist!=null){
						...
						System.out.println("Dont find Selected Item Id");
					}
				}
	...
	public static void addToCart(Integer selectItem,List<Item> items){
		...
		for(Item obj:items){
			if(obj.id==selectItem){
				...
				cart.add(new Cart(obj.itemName,quantity,obj.itemCost,total,obj.id));
	...
	public static int ShowCart(){
		...
		if(cart.isEmpty()){
			...
			int sno=1;
			System.out.println("Sno.\tItem Name\tItem Price\tItem Quantity\tTotal\tItem Id");
			for(Cart obj:cart){
			System.out.println(sno+"\t"+obj.productName+"\t\t"+obj.productPrice+"\t\t"+obj.productQuantity+"\t\t"+obj.totalPrice+"\t\t"+obj.itemId);
			sno++;
			}
		}
		return 1;
	...
	public static void update(Integer itemId,Integer newQuantity){
		
		Cart updateCart=GetCartItem(itemId);
		if(newQuantity<=0){
	...
	public static Cart GetCartItem(Integer itemId){
