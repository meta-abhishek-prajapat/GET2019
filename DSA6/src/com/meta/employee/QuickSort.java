...
public class QuickSort {
	...
	public static int partition(LinkedList<Employee> list, int low, int high) {
		...
		try {
			...
			// Iterating the loop from the first index to high-1 index
			for (int j = low; j < high; j++) {
				Employee node = list.get(j);
				// If this condition satisfies, we increment i and swap with j
			...
			// Swapping with i+1 node -- This is the partition
	...
	public static void sort(LinkedList<Employee> list, int low, int high) {
		if (low < high) {
			...
			// Recursive Calls on before and After the partition
